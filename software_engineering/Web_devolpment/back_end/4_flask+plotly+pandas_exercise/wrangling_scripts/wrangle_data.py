...
# fourth chart shows rural population vs arable land
    ...
    graph_five = []
    df = cleandata(dataset = 'data/API_SP.RUR.TOTL_DS2_en_csv_v2_9914824.csv', keepcolumns = ['Country Name', '2015'], value_variables=['2015'])
    df.columns = ['country','year','ruralpopulation']
    df.sort_values('ruralpopulation', ascending=False, inplace=True)
    country_list = ['United States', 'China', 'Japan', 'Germany', 'United Kingdom', 'India', 'France', 'Brazil', 'Italy', 'Canada']
    df = df[df['country'].isin(country_list)]
    
    graph_five.append(
        go.Bar(
            x = df['country'],
            y = df['ruralpopulation'],
